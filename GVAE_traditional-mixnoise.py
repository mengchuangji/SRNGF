...
def save_hist(x, root):
    x = x.flatten()
    plt.figure()
    n, bins, patches = plt.hist(x, bins=128, density=1) #128
    plt.savefig(root)
    plt.close()
...
def save_compare(clean, noisy, denoise, root, method, epoch):
    ...
    figsize = (12,6)  # 设置图形的大小（12，6）(20,2)
...
def kl(p, q):
    ...
    p_zero_lst = (p == 0)
    q_zero_lst = (q == 0)
    zero_lst = p_zero_lst + q_zero_lst
    zero_idx = [i for i, _ in enumerate(zero_lst) if _]
    p=np.delete(p, zero_idx)
    q = np.delete(q, zero_idx)
    ...
    for i in range(total_step):
        ...
        with torch.no_grad():
            ...
            if gaussian_denoiser == "bm3d":
                ...
                denoise_data =eng.fxydmssa(matlab.double(noisy_data.tolist()),0.0,100.0,0.001,10,1.0,0)  #N=64,10
                # elapsed_time = time.time() - start_time
                # print(' %10s : %2.4f second' % ('fedecon', elapsed_time))
                i0_til_np = np.array(denoise_data)
                i0_til_torch = np_to_torch(i0_til_np).to(device)
                ...
                Y_np = torch_to_np(Y)
                noisy_data = i0_np + Y_np
                denoise_data1 = eng.fxydmssa(matlab.double(noisy_data.tolist()), 0.0, 100.0, 0.001, 24, 1.0, 0); #N=64 10
            ...
            if gaussian_denoiser == "mssa":
                ...
                denoise_data = eng.fx_mssa(matlab.double(noisy_data.tolist()), 1.0, 124.0, 0.001, 10, 0)
                denoise_data = np.array(denoise_data)
                i0_til_np = np.array(denoise_data)
                i0_til_torch = np_to_torch(i0_til_np).to(device)
                # i0_til_torch = torch.Tensor(denoise_data).to(
                #     device)[None, None, ...]
            ...
            n_y=(noise_im.squeeze()-clean_im.squeeze()).flatten()#n_y=y-x
            n_z=(mean_np.squeeze()-clean_im.squeeze()).flatten()#n_z=z-x

            np.random.seed(seed=0)  # for reproducibility
            g_y=np.random.normal(np.mean(n_y), np.var(n_y), n_y.shape)
            np.random.seed(seed=0)  # for reproducibility
            g_z=np.random.normal(np.mean(n_z), np.var(n_z), n_z.shape)
            n_y, bins = np.histogram(n_y, bins=128)
            n_y=n_y/n_y.sum()
            n_z, bins = np.histogram(n_z, bins=128)
            n_z=n_z/n_z.sum()
            g_y, bins = np.histogram(g_y, bins=128)
            g_y=g_y/g_y.sum()
            g_z, bins = np.histogram(g_z, bins=128)
            g_z=g_z/g_z.sum()
            KL_nygy =kl(n_y,g_y)
            KL_nzgz =kl(n_z,g_z)

            n_z_1 = torch_to_np_1C(en_net(noise_im_torch)).squeeze().flatten()
            n_z_1, bins = np.histogram(n_z_1, bins=128)
            n_z_1= n_z_1 / n_z_1.sum()
            np.random.seed(seed=0)  # for reproducibility
            g_z_1 = np.random.normal(np.mean(n_z_1), np.var(n_z_1), n_z_1.shape)
            g_z_1, bins = np.histogram(g_z_1, bins=128)
            g_z_1 = g_z_1 / g_z_1.sum()
            KL_nzgz_1 = kl(n_z_1, g_z_1)
            # KL_nygy=(n_y*np.log(n_y/g_y)).sum()
            # KL_nzgz = (n_z * np.log(n_z/g_z)).sum()
            print('Iteration: {:02d}, KL_nygy: {:f}, KL_nzgz: {:f}'.format(i, KL_nygy, KL_nzgz))
            print('Iteration: {:02d}, KL_nzgz_1: {:f}'.format(i, KL_nzgz_1))
...
if __name__ == "__main__":
    ...
    # synthesis sample
    gaussian_denoiser = "mssa"  # nlm fxdecon bm3d curvelet dmssa localorthoDMSSA mssa
    noisy_name = 'mix-test2'
    case = 2
#############################
    path = '.\seismic\\test\\'
    clean_name = path +'test2-Y.sgy'

    # Choose your Gaussian Denoiser mode
    result_root = '.\output\\GVAE_' + gaussian_denoiser + '_20220511' + noisy_name+'\\'
    ...
    # row = [x for x in range(0, 160)]
    # col = [x for x in range(0, 640)]
    # row=random.shuffle(row)
    # col=random.shuffle(col)
    import random
    ind=[i for i in range(x.shape[0] * x.shape[1])]
    random.shuffle(ind)
    p1=round(0.3*len(ind)) #0.3
    p2 = round(0.6 * len(ind)) #0.6
    p3 = len(ind)-p1-p2

    y=np.zeros(x.shape)
    noise1=np.random.normal(0,0.2, len(ind)) #0,0.2
    noise2 = np.random.normal(-0.1,0.1, len(ind)) #-0.1,0.1
    noise3 = np.random.normal(0.2,0.05, len(ind)) #0.2,0.05
    for i in ind[:p1]:
        y.flat[i]=x.flat[i]+noise1.flat[i]
    for i in ind[p1:p1+p2]:
        y.flat[i]=x.flat[i]+noise2.flat[i]
    for i in ind[p1+p2:]:
        y.flat[i]=x.flat[i]+noise3.flat[i]

    plt.figure()
    n, bins, patches = plt.hist((y-x).flatten(), bins=128, density=1)
    plt.show()
    print('haha')
    ...
    # y=y/y.max()
    ##############################################
    # noisy_im_np=y/abs(y).max()
    # clean_im_np=x/abs(y).max()
    noisy_im_np = y
    clean_im_np = x
