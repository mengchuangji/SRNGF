...
def denoising(noise_im, clean_im, LR=1e-2, sigma=5, rho=1, eta=0.5,
              total_step=20, prob1_iter=500, noise_level=None, result_root=None, f=None,y_max=1):
    input_depth = 1
    latent_dim = 1
    ...
    for i in range(total_step):
        ...
        with torch.no_grad():
            ...
            sio.savemat(('output/results/salt_sc100_GVAE_' + gaussian_denoiser+'_'+str(i)+'_dn.mat'),
                        {'data': (torch_to_np(i0_til_torch)*y_max).squeeze()[:, :]})
...
if __name__ == "__main__":
    ...
    # synthesis sample
    gaussian_denoiser = "dmssa"  # nlm fxdecon bm3d curvelet dmssa localorthoDMSSA mssa
    noisy_name = 'salt-sincos75'
...
#############################
    ...
    with open(result_root + 'result.txt', 'w') as f:
        _, psnr, ssim = denoising(noisy_im_np, clean_im_np, LR=LR, sigma=sigma_, rho=rho, eta=eta,
                                  ...
                                  noise_level=noise_level, result_root=result_root, f=f,y_max=y_max)
