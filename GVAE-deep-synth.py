...
def denoising(noise_im, clean_im, LR=1e-2, sigma=5, rho=1, eta=0.5,
    ...
    for i in range(total_step):
        ...
        with torch.no_grad():
            ...
            sio.savemat(('output/results/salt_sc200_GVAE_' + gaussian_denoiser+'5000_'+str(i)+'_dn.mat'),
...
if __name__ == "__main__":
    ...
    # # #######################
    # import scipy
    # gn=np.random.normal(0, 1, x.shape)
    # plot_83_300(gn, clip=1)
    # op=np.hamming(10).reshape((10, 1))
    # op = np.hamming(10).reshape((10, 1))/sum(sum(op))
    # bpgn = scipy.signal.convolve2d(gn,op,'same')
    # # plot_83_300(bpgn, clip=1)
    # # plot_sns(bpgn, color='red')
    # # plot_sns(bpgn/abs(bpgn).max() * sigma[:, :], color='red')
    # y = x + bpgn/abs(bpgn).max() * sigma[:, :]
