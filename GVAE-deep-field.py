...
def denoising(noise_im, clean_im, LR=1e-2, sigma=5, rho=1, eta=0.5,
    ...
    for i in range(total_step):
        ...
        for i_1 in range(prob1_iter):
            ...
            optimizer.step()
            if i_1  == prob1_iter-1:
                print('D_KL: ', ((1 / sigma ** 2) * l2_loss(en_net(noise_im_torch), i0)).item())
                print('GGVAE_loss ', total_loss.item())
            with torch.no_grad():
                i0 = ((1 / sigma ** 2) * mean.detach() + rho * (i0_til_torch - Y)) / ((1 / sigma ** 2) + rho)
        # elapsed_time_inner = time.time() - start_time
        # print('elapsed_time: %2.4f second' % (elapsed_time_inner))
        with torch.no_grad():
            ################################# sub-problem 2 ###############################
            ...
            print('elapsed_time: %2.4f second' % (elapsed_time_outer))
            ###############################################################################
            ...
            # plot_3_300(noise_im.squeeze()-torch_to_np(i0_til_torch))
            # plot_3_300_jet(simi.squeeze())

            # fn = (noise_im.squeeze() - torch_to_np(i0_til_torch)).copy().flatten()
            # import matplotlib.pyplot as plt
            # plt.figure(dpi=300, figsize=(3, 3))
            ...
            # x__ = noise_im.squeeze().copy()[30:94, 0:64]  #[280:, 300:364]  [50:100, 0:64] [340:, 300:340]
            # denoised__ = torch_to_np(i0_til_torch).copy()[30:94, 0:64]#[50:100, 0:64]
...
if __name__ == "__main__":
    ...
    for noise, clean, in zip(noises, cleans):
        ...
        noise_im_np=noise_im_np#[:,32:96,:64]
        clean_im_np=clean_im_np#[:,32:96,:64]
        plot_3_300(noise_im_np.squeeze())
        plot_3_300(clean_im_np.squeeze())
