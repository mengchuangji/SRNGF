    ...
    for i in range(total_step):
        ...
        with torch.no_grad():
            ...
            if gaussian_denoiser == "bm3d":
                ...
                Y_np = torch_to_np(Y)
                noisy_data=i0_np + Y_np
                sig = float(eval_sigma(i + 1, noise_level))
                denoise_data = eng.seismic_curvelet_denoise(matlab.double(noisy_data.tolist()),sig);
                # elapsed_time = time.time() - start_time
                # print(' %10s : %2.4f second' % ('fedecon', elapsed_time))
                i0_til_np = np.array(denoise_data)
                i0_til_torch = np_to_torch(i0_til_np).to(device)
            ...
            sio.savemat(('../../output/results/salt_sc100_vae_' + gaussian_denoiser + '_' + str(i) + '_dn.mat'),
...
if __name__ == "__main__":
    ...
    # synthesis sample
    gaussian_denoiser = "curvelet"  # nlm fxdecon bm3d curvelet dmssa localorthoDMSSA
