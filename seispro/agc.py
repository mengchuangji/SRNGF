...
def agc(data, time_window_side_len=64):
    ...
    AGC is a commonly used process to reduce the amplitude range of test
    ...
    Inputs:
        test: A Tensor of shape [batch_size, n_traces, n_times]
        time_window_side_len: An integer controlling the number of samples in
                              the final dimension of the test (usually time
                              or depth) to use when calculating the scaling.
                              The number used will be 2*trace_window_len+1.

    Returns:
        test: The input test after AGC
        scaling: A Tensor of the same shape as the test, containing the
                 scaling applied to each sample. This can be used to
                 undo AGC (by dividing the test by the scaling).
    """
...
def agc_jit(data, time_window_side_len):
    ...
    # [batch_size, channels, conv_dim] for the test,
