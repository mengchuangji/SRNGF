...
def show(x,y,x_):
    ...
    vmin, vmax = -clip, clip
    plt.figure(dpi=500,figsize=(16, 30))  # 16,3
    plt.subplot(611)
    plt.xticks([])  # 去掉横坐标值
    plt.yticks([])  # 去掉纵坐标值
...
# %     D 1.0,124.0,0.001,9,0
# %  OUT  D1:  	output data
import time
start_time = time.time()
denoise_data = eng.fx_mssa(matlab.double(noisy_data.tolist()),1.0,100.0,0.004,16,0) #0.004 10
denoise_data=np.array(denoise_data)
elapsed_time = time.time() - start_time
print(' %10s : %2.4f second' % ('mssa', elapsed_time))
print('haha')
...
# axs[0].imshow(data_test,  vmin=vmin, vmax=vmax)#cmap=plt.cm.seismic,
# axs[0].set_title('Clean')
#
# axs[1].imshow(noisy_data, vmin=vmin, vmax=vmax) #, cmap=plt.cm.seismic
...
#
# axs[2].imshow(denoise_data,  vmin=vmin, vmax=vmax)#cmap=plt.cm.seismic,
...
#
# axs[3].imshow(noise_data,  vmin=vmin, vmax=vmax) #cmap=plt.cm.seismic,
# # Denoised_psnr = psnr(data_test,denoise_data)
# # Denoised_psnr=round(Denoised_psnr, 2)
...
# show_DnNR_1x3(x=data_test,y=noisy_data,x_=denoise_data,method='mssa')
show_DnNR_f_1x3_(x=data_test,y=noisy_data,x_=denoise_data,method='mssa')
...
show_DnNSimi_f_1x3_(x=data_test,y=noisy_data,x_=denoise_data,simi=simi.squeeze(),method='mssa')