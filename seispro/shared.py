...
def extract_trace_windows(data_fx, trace_window_starts, trace_window_len):
    ...
    """Extracts windows of traces from the test.
...
def inverse_fourier_transform_time(data_fx, time_window_len, n_times):
    ...
    #test = torchaudio.functional.istft(
    data = torch.istft(        
        data_fx,
        time_window_len,
...
def fourier_transform_time(data, time_window_len):
    ...
    """Windows and Fourier transforms the test in time.
    ...
    """
    # Use the Short-Time Fourier Transform (STFT) to window test in time and
    # Fourier transform. This requires that the test is in 2D, with time
