...
if __name__ == '__main__':
    ...
    # revise
    checkpoint = torch.load('./models_denoise/1111/fre/f_s_7/13reviseVUn/model_state_15')
    ...
    # original = readsegy(data_dir, '03-MonoNoiAtten-16_DYN_L1901-s11857.sgy')[400:1600, 0:224]
    # original = readsegy_ith_agc(data_dir2, 'BGP_00_huangtuyuan_pao.sgy', 19, trace_per_shot=224, agc=True)[224:1600, 0:224]  #

    original = readsegy_ith_agc(data_dir3, 'shots0001_0200.segy', 1, trace_per_shot=1201, agc=False)[:960,600:1201]#2001*1201
    original=original/abs(original).max()
    x=(original+np.random.normal(0, 0.01, original.shape)).astype(np.float32)
    ...
    if args.save_result:
        ...
        show_yxn_gain(x, denoised, agc=False)
