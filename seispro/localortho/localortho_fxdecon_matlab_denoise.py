...
file_list1='../../seismic/field/00-L120-X.sgy'
f = segyio.open(file_list1, ignore_geometry=True)
f.mmap()#mmap将一个文件或者其它对象映射进内存，加快读取速度
data = np.asarray([np.copy(x) for x in f.trace[:]]).T#[600:728,:128] #(512,512)
...
f.mmap()#mmap将一个文件或者其它对象映射进内存，加快读取速度
data = np.asarray([np.copy(x) for x in f.trace[:]]).T#[600:728,:128]   #[:160, :640]#(512,512)
...
denoise_data1 = eng.fx_decon(matlab.double(noisy_data.tolist()),matlab.double([0.004]),matlab.double([15]),matlab.double([0.01]),matlab.double([1]),matlab.double([124])); #XJ 0.004 PK 0.002 salt 0.001 100
...
# show_DnNR_1x3(x=data_test,y=noisy_data,x_=denoise_data,method='lo+fxdecon')

plt.figure(dpi=300, figsize=(3, 3))
plt.imshow(denoise_data, vmin=-1, vmax=1, cmap=plt.cm.seismic)
# plt.title('denoised')
plt.axis('off')
plt.show()
plt.figure(dpi=300, figsize=(3, 3))
plt.imshow(data_test-denoise_data, vmin=-1, vmax=1, cmap=plt.cm.seismic)
# plt.title('denoised')
plt.axis('off')
plt.show()