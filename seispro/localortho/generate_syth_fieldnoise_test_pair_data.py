...
#  该函数  加了  *N.s*gy
XJ_noise_list = generate_patch_from_poststack_segy_1by1(dir=XJ_segy_dir, pch_size=(256,256),stride=(128,128),jump=1,agc=False,train_data_num=100000,trace_num=20000,section_num=40,aug_times=[],scales = [])
# ori_max=abs(XJ_im_list).max()
XJ_noise_list=np.concatenate([XJ_noise_list, XJ_noise_list,XJ_noise_list], axis=0)
shape0_min=min(marmousi_im_list.shape[0],XJ_noise_list.shape[0])
test_clean_im_list=marmousi_im_list[0:shape0_min,:,:]
test_noisy_im_list=marmousi_im_list[0:shape0_min,:,:]+XJ_noise_list[0:shape0_min,:,:]
path_h5=marmousi_im_dir
path_h5 = os.path.join(path_h5, 'marmousi20mp_tst_XJ_s110_120_noise_Patches_256.hdf5')#
...
with h5.File(path_h5, 'w') as h5_file:
    for ii in range(len(test_noisy_im_list)):
        if (ii+1) % 10 == 0:
            print('    The {:d} original images'.format(ii+1))
        pch_noisy = test_noisy_im_list[ii].squeeze() #/ori_max
        pch_clean = test_clean_im_list[ii].squeeze()
        if ii == 0:
            show(pch_noisy,pch_clean,method='orthofxdecon')
        # noise_data=pch_noisy-pch_dn
        pch_imgs = np.concatenate((np.expand_dims(pch_noisy,axis=2), np.expand_dims(pch_clean,axis=2)), axis=2)
