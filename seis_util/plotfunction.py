...
def show_DnNR_1x3(x,y,x_,method):
    ...
    labelfontsize=20
    clip =1
    x_max=abs(x).max()  # 显示范围，负值越大越明显  用x好一些
    vmin, vmax = -clip, clip
    fig=plt.figure(dpi=300,figsize=(26, 3))  # 16,3 (26, 3)
    plt.rcParams["font.family"] = "Times New Roman"
    # fig.suptitle("FXDECONV", y=1.05, fontsize=18) #总标题y=-0.2
    # ####################
    snr_x_ = compare_SNR(x, y)
    ...
    plt.imshow(x_/x_max, vmin=vmin, vmax=vmax, cmap=plt.cm.seismic) # x_
    ...
    plt.imshow(noise/x_max, vmin=vmin, vmax=vmax, cmap=plt.cm.seismic) #noise
    ...
    ax.yaxis.set_label_coords(-.02, .5)
    plt.tick_params(axis='both', which='major', labelsize=labelfontsize, labelbottom=False, bottom=False, top=True, labeltop=True)
    plt.imshow((x_ - x)/x_max, vmin=vmin, vmax=vmax, cmap=plt.cm.seismic)
...
def show_DnNR_f_1x3_(x,y,x_,method):
    ...
    clip =1 #abs(x).max()  # 显示范围，负值越大越明显  用x好一些
    vmin, vmax = -clip, clip
    fig=plt.figure(dpi=300,figsize=(17, 5))  # 16,3 (26, 3)dpi=500 (18,5)
    ...
    snr_x_ = compare_SNR(x, x_)
    print(' snr_after= {1:2.2f}dB'.format('test', snr_x_))
    psnr_x_ = compare_psnr(x, x_)#datarange=2*abs(y).max
    print('psnr_after=', '{:.4f}'.format(psnr_x_))
    ssim = compare_ssim(x, x_)
    print('ssim_after=', '{:.4f}'.format(ssim))
    ####################################################
...
def show_DnNSimi_f_1x3_(x, y, x_,simi, method):
