...
from seis_util.download_data import *
...
class FieldDataset(Dataset):
    ...
    """
    def __init__(self, ys, xs,ys_max):
    ...
    def __getitem__(self, index):
        batch_x = self.xs[index]/self.ys_max
        batch_y = self.ys[index]/self.ys_max
        torch.set_default_dtype(torch.float64)
        return batch_y, batch_x
    ...
    for i in range(len(file_list)):
        with segyio.open(file_list[i],'r',ignore_geometry=True) as f:
            ...
            for j in range(0,shot_num,jump):
                ...
                data = np.asarray([np.copy(x) for x in f.trace[0:2500]]).T
                q += 1
                if agc:
