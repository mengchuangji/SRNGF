...
def denoising(noise_im, clean_im, LR=1e-2, sigma=5, rho=1, eta=0.5,
              total_step=20, prob1_iter=500,  result_root=None, f=None,y_max=1):
    input_depth = 1
    latent_dim = 1
    ...
    for i in range(total_step):
        ...
        for i_1 in range(prob1_iter):
            ...
            epsilon = torch.randn_like(mean).to(device)#/255 #这里不除于255更好
        ...
        plot_83_300(torch_to_np(mean),clip=1)
        plot_sns(torch_to_np(mean) - clean_im.squeeze(),color='blue')  # z-x
        plot_83_300(torch_to_np(out),clip=1)
        plot_83_300(torch_to_np(i0),clip=1)  # x_{i+1}
        plot_83_300(torch_to_np(i0 + Y),clip=1)
        plot_sns(torch_to_np(i0 + Y) - clean_im.squeeze(),color='blue')  # y-x
        ...
        with torch.no_grad():
            ...
            Y = Y + eta * (i0 - i0_til_torch)#/255  # / 255 这里取值多少不影响
            ...
            i0_til_np = torch_to_np(i0_til_torch)#.clip(-1, 1)#.clip(-y_max,y_max)##.clip(0, 255)
            # save_np(i0_til_np,os.path.join(result_root, '{}'.format(i) + '.png'))
            psnr = compare_psnr(clean_im.squeeze(), torch_to_np(i0_til_torch).squeeze()) #,data_range=y_max
            ...
            # noise_=noise_im.squeeze()-torch_to_np(i0_til_torch).squeeze()
            # sio.savemat(('output/results/salt_sc200_GVAE' + gaussian_denoiser + '_' + str(i) + '_n.mat'),
            #             {'data': noise_[:, :]})
...
def plot_83_300(x,clip):
    plt.figure(dpi=300, figsize=(8, 3))
    plt.imshow(x, vmin=-clip, vmax=clip, cmap=plt.cm.seismic)
    plt.axis('off')
    plt.show()
...
if __name__ == "__main__":
    ...
    sigma = 10 / 255.0 + (sigma - sigma.min()) / (sigma.max() - sigma.min()) * ((300 - 10) / 255.0)
    sigma = cv2.resize(sigma, (W, H))
    np.random.seed(seed=0)  # for reproducibility

    data_dir = 'E:\博士期间资料\田亚军\田博给的数据\\20220128'
    data_dir2 ='E:\博士期间资料\田亚军\田博给的数据\\2021.3.27数据'
    data_dir3= 'E:\博士期间资料\田亚军\田博给的数据\\2021.6.07 新疆数据'
    from seis_util.readsegy import readsegy_agc,readsegy
    # FN=readsegy(data_dir2, '03-MonoNoiAtten-16_DYN_L1901-s11857.sgy')[200:360, 0:224]  # [400:800, 0:224]
    # FN=readsegy_agc(data_dir, 'BGP_00_huangtuyuan_pao.sgy',agc=False)[200:360, 0:640]  # [400:800, 0:224]

    FN= readsegy_agc(data_dir3, '00-N.sgy',agc=False)[80:240, 0:640]
    # FN =np.tile(FN, (1, 3))[:,:640]
    FN=FN/abs(FN).max()
    def ind2sub(array_shape, ind):
        ...
        return [rows, cols]
    idx_= ind2sub(FN.shape, 161)
    idx_p=np.random.permutation(range(FN.shape[0] * FN.shape[1]))
    len1 = int(np.floor(FN.shape[0]* FN.shape[1] * 0.05))
    len2 = int(np.floor(FN.shape[0]* FN.shape[1] * 0.3))
    len3 = FN.shape[0]* FN.shape[1] - len1 - len2
    idx1=[ind2sub(FN.shape, i) for i in idx_p[0:len1]]
    idx2 =[ind2sub(FN.shape, i) for i in idx_p[len1:len1+len2]]
    idx3 =[ind2sub(FN.shape, i) for i in idx_p[len1+len2:]]
    row_idx1, col_idx1 = [s[0] for s in idx1],[s[1] for s in idx1]
    row_idx2, col_idx2 = [s[0] for s in idx2], [s[1] for s in idx2]
    row_idx3, col_idx3 = [s[0] for s in idx3], [s[1] for s in idx3]
    y=x.copy()
    y[row_idx1, col_idx1]=y[row_idx1, col_idx1]+np.random.normal(0.1,0.5, x[row_idx1, col_idx1].shape)# * sigma[:, :]
    y[row_idx2, col_idx2] = y[row_idx2, col_idx2] + np.random.normal(-0.01,0.1, x[row_idx2, col_idx2].shape)  # * sigma[:, :]
    y[row_idx3, col_idx3] = y[row_idx3, col_idx3] + np.random.normal(0,0.2, x[row_idx3, col_idx3].shape)  # * sigma[:, :]

    # 预处理
    # for i in range(FN.shape[1]):
    #     from seis_util.utils import butter_bandpass_filter
    #     x[:,i]=butter_bandpass_filter(x[:,i], lowcut=60, highcut=200, fs=1/0.002, order=5)
    # plot_83_300(FN, clip=0.1)
    # hty_col_max = np.amax(FN, axis=0)
    # for i in np.where(hty_col_max > 3e-1)[0].tolist():
    #     # plot_1D_magnitude_spectrum(x,i)
    #     x[:, i] = x[:, i] * 0.3
    #     # x[:,i]=x[:,i]*0.001
    #     # x[:, i] = 0
    # print(hty_col_max)

    plot_83_300(y-x,clip=1)
    plot_sns(y-x,color='red')
    # # #######################
    # y = x + 3*FN * sigma[:, :]
    # # y = x + FN
    plot_83_300(y, clip=1)
    # plot_sns(3*FN * sigma[:, :], color='red')
    # ################################
    y_max=abs(y).max()
    y_max=1
    y = y/y_max
    ...
    # sio.savemat(('output/results/salt.mat'), {'data': x[:, :]})
    # sio.savemat(('output/results/salt_g30.mat'), {'data': y[:, :]})
